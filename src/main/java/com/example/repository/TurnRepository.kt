...
class TurnRepository(private val turnDao: TurnDao) {
    ...
    private fun getFullAchievementsList(userId: Int): List<AchievementEntity> {
        return listOf(
            ...
            // 68-88: Advanced Milestones, Random Wheel, Backup & Morning Warbler (88 total)
            AchievementEntity(userId, "early_morning_warbler", "الغرّيد الباكر 🌅", "أنهِ دوراً في الصباح بين ٦ إلى ٩ صباحاً", false, 0, 1),
            AchievementEntity(userId, "wheel_spinner", "سيد العجلة 🎡", "قم بتدوير عجلة الاختيار العشوائي", false, 0, 1),
            AchievementEntity(userId, "wheel_champion", "بطل القرعة 🎲", "ابدأ دوراً من خلال عجلة الاختيار العشوائي", false, 0, 1),
            AchievementEntity(userId, "backup_master", "حارس البيانات 💾", "أنشئ نسخة احتياطية لبياناتك بنجاح", false, 0, 1),
            AchievementEntity(userId, "restore_pioneer", "صانع المعجزات 🔄", "استعد نسخة احتياطية من ملف محمي بنجاح", false, 0, 1),
            AchievementEntity(userId, "about_explorer", "صديق المطور 📱", "قم بزيارة شاشة 'عن التطبيق' وتعرف على المطور", false, 0, 1),
            AchievementEntity(userId, "retro_enthusiast", "حنين الكلاسيك 👾", "جرّب وطبّق ثيم أندرويد كلاسيكي", false, 0, 1),
            AchievementEntity(userId, "super_amoled_fan", "عاشق السواد المطلق 🕶️", "استخدم وطبّق ثيم شاشة سوبر أموليد", false, 0, 1),
            AchievementEntity(userId, "neon_cyber", "سايبر نيون 🔮", "استخدم وطبّق ثيم النيون المستقبلي", false, 0, 1),
            AchievementEntity(userId, "smart_responder", "المستجيب الذكي 🤖", "ابدأ دوراً عبر نظام الإشعارات الذكي", false, 0, 1),
            AchievementEntity(userId, "lunch_break_hero", "استراحة الغداء 🥪", "أنهِ دوراً بين الساعة 1:00 و 2:00 ظهراً", false, 0, 1),
            AchievementEntity(userId, "sunset_focus", "هدوء الغروب 🌇", "أكمل دوراً بين الساعة 5:00 و 6:00 مساءً", false, 0, 1),
            AchievementEntity(userId, "midnight_scholar", "مفكر منتصف الليل 🌌", "أكمل دور دراسة بعد الساعة 11:00 ليلاً", false, 0, 1),
            AchievementEntity(userId, "century_minutes", "نادي الـ100 دقيقة ⏱️", "أكمل دوراً واحداً مدته تتجاوز 100 دقيقة", false, 0, 1),
            AchievementEntity(userId, "five_minute_sprint", "سباق الـ5 دقائق ⚡", "أكمل دوراً مدته بالضبط أو أكثر من 5 دقائق", false, 0, 1),
            AchievementEntity(userId, "half_hour_flow", "تدفق النصف ساعة 🌊", "أكمل دوراً مدته 30 دقيقة بنجاح", false, 0, 1),
            AchievementEntity(userId, "day_streak_60", "أسطورة الستين يوماً 🏆", "مارس الأدوار في 60 يوماً مختلفاً", false, 0, 60),
            AchievementEntity(userId, "day_streak_100", "مئوية الالتزام الأسطوري 🌟", "مارس الأدوار في 100 يوم مختلف", false, 0, 100),
            AchievementEntity(userId, "marathon_day", "يوم الماراثون الفولاذي 🥇", "أكمل 15 دوراً في نفس اليوم", false, 0, 15),
            AchievementEntity(userId, "deep_focus_warrior", "محارب التركيز العميق 🧘‍♀️", "أكمل 3 أدوار مدة كل منها أكثر من 25 دقيقة", false, 0, 3),
            AchievementEntity(userId, "multi_user_maestro", "مايسترو المجموعات 👥", "أدر جلسة تضم 5 لاعبين مسجلين أو أكثر", false, 0, 1)
        )
    }
    ...
    private suspend fun updateAchievementsAfterTurn(
        ...
        if (elapsedSeconds >= 300) {
            unlockUserAchievement(userId, "five_minute_sprint")
        }
        ...
        if (elapsedSeconds >= 1800) {
            unlockUserAchievement(userId, "half_hour_flow")
        }
        if (elapsedSeconds >= 3600) {
            unlockUserAchievement(userId, "patient_monk")
        }
        if (elapsedSeconds >= 6000) {
            unlockUserAchievement(userId, "century_minutes")
        }
        if (elapsedSeconds >= 7200) {
            unlockUserAchievement(userId, "ultra_marathon")
        }

        // Deep focus warrior (count turns >= 1500s / 25 mins)
        if (elapsedSeconds >= 1500) {
            achs.find { it.id == "deep_focus_warrior" }?.let { ach ->
                if (!ach.isUnlocked) {
                    val p = (ach.progress + 1).coerceAtMost(3)
                    turnDao.updateAchievement(ach.copy(progress = p, isUnlocked = p >= 3))
                }
            }
        }
        ...
        // "الغرّيد الباكر. أنه دوراً في الصباح بين ٦ إلى ٩ صباحاً"
        if (hour in 6..9) {
            unlockUserAchievement(userId, "early_morning_warbler")
        }
        if (hour in 12..15) {
            unlockUserAchievement(userId, "afternoon_rush")
        }
        if (hour in 13..14) {
            unlockUserAchievement(userId, "lunch_break_hero")
        }
        if (hour in 16..18) {
            unlockUserAchievement(userId, "tea_time")
        }
        if (hour in 17..18) {
            unlockUserAchievement(userId, "sunset_focus")
        }
        if (hour in 19..23) {
            unlockUserAchievement(userId, "evening_relax")
        }
        if (hour >= 23 && sessionType == "STUDY") {
            unlockUserAchievement(userId, "midnight_scholar")
        }
        ...
        achs.find { it.id == "marathon_day" }?.let { ach ->
            if (!ach.isUnlocked) {
                val p = turnsToday.coerceAtMost(15)
                turnDao.updateAchievement(ach.copy(progress = p, isUnlocked = p >= 15))
            }
        }
        ...
        if (allUsersInDb.size >= 5) {
            unlockUserAchievement(userId, "multi_user_maestro")
        }
    ...
    suspend fun onRandomWheelSpun(userId: Int) {
        unlockUserAchievement(userId, "wheel_spinner")
    }

    suspend fun onRandomWheelTurnStarted(userId: Int) {
        unlockUserAchievement(userId, "wheel_champion")
    }
    ...
    suspend fun onSpecificThemeApplied(userId: Int, themeKey: String) {
        when (themeKey.uppercase()) {
            "RETRO" -> unlockUserAchievement(userId, "retro_enthusiast")
            "AMOLED" -> unlockUserAchievement(userId, "super_amoled_fan")
            "NEON" -> unlockUserAchievement(userId, "neon_cyber")
        }
    }

    suspend fun onSmartNotificationUsed(userId: Int) {
        unlockUserAchievement(userId, "smart_responder")
    }
